...
export const products=[{id:0,image:apple, aa:
  ...
  {id:3,image:sm, para:'  Featuring a vibrant touch display, it tracks your fitness goals, monitors heart rate, and provides notifications right on your wrist. With customizable watch faces and interchangeable band.',aa:'S9 Ultra Series 9 Smart Watch|Body Temperature Monitor|Bluetooth Call|Calculator Wireless ',price:'Rs.20000'},
    {id:4,image:smart, price:'Rs.10000', para:'Featuring a vibrant touch display, it tracks your fitness goals, monitors heart rate, and provides notifications right on your wrist. With customizable watch faces and interchangeable bands, you can express your personal style effortlessly.', aa:'S8 Ultra Series 8 Smart Watch|Body Temperature Monitor'
      ...
      image:hp, aa:'Hp Chromebook 11 G6 EE | 4GB RAM DDR4 | 16GB SSD EMMC ', para:'Discover the HP Chromebook 11 G6 EE, a reliable and compact laptop designed for everyday tasks. With 4GB of DDR4 RAM and 16GB eMMC storage, it offers smooth performance. ', price:'Rs.50000'
...
// }
  return (
   ...
   <div className='child' key={elem.id}><div className="img">   <Link to={`/productpage/${elem.id}`}><img src={elem.image} alt="" style={{height:'200px',width:'200px'}}/> </Link></div>
   <div className="descrip">
    <div className="headp"><h4>
    <Link to={`/productpage/${elem.id}`}>{elem.aa}</Link></h4>
   {/* <div className='pa'><p style={{color:'black'}}>{elem.para}</p> </div> */}
   <div style={{display:'flex'}}><FontAwesomeIcon style={{color:'orange'}} icon={faStar} /><FontAwesomeIcon style={{color:'orange'}} icon={faStar} /><FontAwesomeIcon  style={{color:'orange'}} icon={faStar} /><FontAwesomeIcon style={{color:'orange'}} icon={faStar} />
   <FontAwesomeIcon style={{color:'orange'}} icon={faStarHalfStroke} /></div> <div><p>50+ has been sold out</p></div><div><p>Available only for Residents of Pakistan</p></div><div className='sp'><span >{elem.price}</span></div>
  </div><div className="bton"><button  onClick={()=>addtocart(elem)}>Add to Cart</button></div></div></div>
      ))}
 </div>
 )
}
